...
botmsg = await channel.send('{line[6:]}')"""
                ...
                if status[1] == "playing":
                    stat = 0
                if status[1] == "listening":
                    stat = 2
                if status[1] == "watching":
                    stat = 3

                cmdscript = cmdscript + "client.user.setPresence({game: {name:" + f"""'{status[0]}',type: {stat}}})"""
...
@atexit.register
def dele():
    if os.path.exists("runbot.txt"):
        os.remove("runbot.txt")
